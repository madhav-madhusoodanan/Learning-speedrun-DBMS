...
func GenerateFakeSwaps(ctx context.Context, conn *pgx.Conn, numSwaps int) error {
    ...
    for i := 0; i < numSwaps; i++ {
		...
		// queries, err := NewDatabase(dbConfig)
		queries := New(conn)
...
func RunCrossChainFaker() {
    ...
    // Setup connection (replace with your connection details)
    conn, err := pgx.Connect(ctx, "postgres://postgres:postgres@localhost:5432/rampx")
    if err != nil {
        panic(err)
    }
    ...
    err = GenerateFakeSwaps(ctx, conn, 100)
    if err != nil {
        panic(err)
    }
}